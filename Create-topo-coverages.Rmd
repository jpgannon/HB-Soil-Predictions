...
#"ta_hydrology"
#convert bedrock weight raster to saga file format
rast(paste0(data_dir,"BRweights.tif")) %>% 
  terra::writeRaster(filename = paste0(data_dir, "bossBR.sdat"),
              filetype = "SAGA")

rsaga.get.modules(libs = "ta_hydrology",
                  env = sagaenv)

#set parameters (https://saga-gis.sourceforge.io/saga_tool_doc/2.3.0/ta_hydrology_0.html)
params = list(ELEVATION = paste0(data_dir, "dem5m.sgrd"),
              WEIGHTS = paste0(data_dir, "BRweights.sdat"),
              METHOD = 4,
              FLOW = paste0(data_dir, "UAAb.sdat"))
...
#read in the result of above, assign projection, write back out projected
UAAb <- rast(paste0(data_dir, "UAAb.sdat"))
crs(UAAb) <- "epsg:26919"
writeRaster(UAAb, paste0(data_dir, "UAAb.tif"), overwrite = TRUE)
